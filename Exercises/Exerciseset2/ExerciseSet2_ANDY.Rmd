...
dowjones/sp500.emini
Eurostock/Dax30 and Cac40

Canadian Dollar/Australian dollar are negative correlated. Sell one, buy the other?
...
1. The first SIX
<br><br>
2. The highest weights (in absolute value) of the first principal component portfolio is 21.94
<br><br>
3. Yes, I would call them the followin:
1. GOV BONDS of DEVELOP COUNTRIES
2. CURRENCIES of DEVELOP COUNTRIES
3. EUROPEAN STOCK INDEXES
4. NORTH AMERICAN STOCK INDEXES
5. EUROPEAN f/x FUTUTES & SECURITIES INDEX
6. GAS & OIL
7. NON-FERROUS METAL INDEX
8. CROP COMODITIES 
9. PRECIOUS METALS 
10.EAST ASIAN STOCK INDEXES
<br><br>
4.
a) Pre-crisis bull market years (November 1, 2002, and October 1, 2007)
...
m1_pre_crisis<-gvisTable(show_data_pre_crisis,options=list(width=1920, height=min(400,27*(nrow(show_data_pre_crisis)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1_pre_crisis,'chart')

#Screen Plot

eigenvalues_pre_crisis  <- Variance_Explained_Table_pre_crisis[,2]
...
#Rotated principal components - Pre-Crisis

corused_pre_crisis = cor(pre_crisis_data[,apply(pre_crisis_data!=0,2,sum) > 10, drop=F])
Rotated_Results_pre_crisis<-principal(corused_pre_crisis, nfactors=10, rotate="varimax",score=TRUE)
...
colnames(Rotated_Factors_pre_crisis)<-paste("Component",1:ncol(Rotated_Factors_pre_crisis),sep=" ")
...
colnames(show_data_pre_crisis)<-c("Variables",colnames(Rotated_Factors_pre_crisis))
m1_pre_crisis<-gvisTable(show_data_pre_crisis,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data_pre_crisis)+1)),allowHTML=TRUE,page='disable'))
print(m1_pre_crisis,'chart')


```

b) Crisis market years (October 1, 2007, and Marzo 1, 2009)
...
#Rotated principal components - Crisis
...
```
<hr><hr>
5. Components calculated for each of the datasets (bull, crisis, all years) are different. The correlation of futures increases during financial crisis. Pre-crisis, 6 first components are resposnible for more than 50% of variance. Crisis, 4 first components are resposnible for more than 50%.  
...
**Your Answers here:**
<br>
1. The code above initiates a matrix with 64 columns and 3836 rows (corresponding to days between 2001 and 2015) and it creates a list of moving averages. At the end of the process, for every entry the program assess whether the return of every cell (dates and attributes) was positive or negative (indicated by a plus or minus). 
<br>
<br>
2. The command line MAfreq<-seq(10,250,by=20), affects how often the trading strategy is implemented. For instance, a shorter sequence with tighter values (i.e. 10,15,20...) or a longer sequence with less tighert values (i.e 10,40,70...)
<br>
