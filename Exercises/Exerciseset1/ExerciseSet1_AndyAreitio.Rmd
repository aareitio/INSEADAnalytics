...
author: "T. Evgeniou + Andy Areitio"
...
**Your Answers here:**
a) Here (https://github.com/R-Finance) there are around 15 finance-related R github repositories.
<br>
b) Here (https://github.com/bergant/finstr) there are  R github repositories ready to create an environment for reproducible financial statement analysis. 
<hr>
<hr>
...
**Your Answers here:**
<br>
1. Yes,In my console I can see the following:
"\nDownloading ticker  AAPL  ..."
The code is in the `dataset1.R` file. --> `mytickers = c("SPY", "AAPL")`
<br>
<br>
...
<br>
<br>
3. Just modify the `dataset1.R` file line to `mytickers = c("SPY", "AAPL","YHOO")` and then do for Yahoo! exactly what we did for Apple in Question 2. 
<br>
Cumulative Daily Returns
...
**Your Answers here:**
<br>
1. The commands I need is dim(x) or nrow (x) and ncol (x). It will be like follow:
```{r eval = FALSE, echo=TRUE}
...
```
<br>
class(transposedData) = "matrix" <br>
dim(transposedData) = 2 4037 <br>
nrow(transposedData) = 2 <br>
ncol(transposedData) = 4037 <br>
<br><br>
2. Yes, I just need to be careful with the Margin (=the vector giving the subscripts which the function will be applied over). (E.g. 2 indicates columns.)
```{r eval = FALSE, echo=TRUE}
transposedData = t(StockReturns)
...
<br>
<br>
1. Yes. Go to the file dataSet1.R and change the expression startDate to get = "2001-01-01". StockReturns are calculated using Stock Prices from past days. Now, we rerun again the document.
<br>
<br>
2. Yes, go to the file dataSet1.R and add more companies to the expression mytickers = c("SPY", "AAPL", "YHOO", "GOOG") and adjust the portfolio using the mean function. Now, we rerun again the document.
<hr>
<hr>
...
<br>
<br>
1. When I run the command sum(myData != StockReturns[1:20,]), the console gives me back the number "20".
<br>
<br>
2. When I run the command myData + StockReturns[1:40,], I get the following error: "error in FUN(left, right) : non-numeric argument to binary operator". It seems to me that the reasons is that StockReturns[1:20] has 20 rows and StockReturns[1:40,] has 40 rows. So the program can not add them up.
<hr>
<hr>
...
Yes, I can. I have uploaded the file of the Boats case (Boats.csv). I added this piece of code:
```{r eval = TRUE, echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis'}
#This is the answer to the extra question! 
#I simply want to know how many colums and rows has the file, and the Mean and the StandDev of a particular colum.
myData_boats <- read.csv(file = "boats.csv", header = TRUE, sep=";")
class(myData_boats)
dim(myData_boats)
nrow(myData_boats)
ncol(myData_boats)
round(100*mean(myData_boats[,2]),3)
round(100*sd(myData_boats[,2]),3)
```
