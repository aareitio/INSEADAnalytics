...
* The factors we select describe the attributes different customers have and thus can be used to group customers in sensible segments. We interpret them as 

*Question 4) 

* From the factor analyisis we learn that it is important to first understand and then simplify data when we do an analyisis. We cannot treat data analysis as a "black box"; rather, we have to give business sense to each step in our analyisis. 
...
*Person "Obs 1" scores very highly on Factors 2 and 4 and very low on 5 and 6. This means he is a very trendy Renaissance male that is not price sensitive and does not like to repair/build his boat himself. 

*Person "Obs. 2" is very similar to "Obs 1", however his scores are not as extreme

*Person "Obs. 3" is very technology and trend oriented, and does not care at all for price. She also doesn't like to do works herself.

*Person "Obs 6" is a passionate outdoor enthusiast, who is not price sensitive
*
*Person "Obs 10" is very very price sensitive; however, he doesn't like to DIY at all. He also doesn't care for the latest technology, but is very passionate and an outdoor enthusiast
*
Question 2)
...
# Please use numbers, not column names, e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
segmentation_attributes_used = c(28,14,20,8,3,12) #c(28,14,20,8,3,12) 
...
# We changed it to see 4 variables instead of 2

```{r}
variables_to_plot = 1:4
...
*The numbers show the average scaled score of each segment relative to the average of the population. 
*This represents the distance of the segment to the population, i.e., if the segment differs from the average in that particular aspect. 
*The questions 1-29 (segmenting questions) show larger deviations than the other questions (profiling questions). Q1-29 define the segment, while the profiling questions only show further attributes of the respective segment (e.g., average age, sex, etc.)

2. 

*Segment 1 consists of people who worry about price more than brands. Boating is their real passion in life. They are knowledgable about boats and like to performe minor repairs/maintainance on his own. 

*Segment 2 contains professionals, e.g., fishermen. They are also value conscious and will chose functionality above design. They do not care as much about status and trends. 

*Segment 3 contains hobby sailors - people that do not care too much about any of the attributes and have less experience. They would not perform any repairs on their own. Likely, this might be spouses or friends of sailors that only come for the social aspect and are unlikely to buy a boat on their own.

*Segment 4 responds highly to Q2 and Q9. That means they care little for brands but a lot for the latest and greatest and see their boats as a status symbol. That seems contradictory - we need to investigate this segment further to understand exactly what it is about. 

*Segment 5's key feature is that they don't perform any repairs on their own. However, they consider themselves knowledgable and others seek their advice. Possibly, these are very wealthy customers. However, it needs to be investigated what their needs are.


*Overall, we can make sense out of the segmentation, but some of the segment attributes seem contradictory. We would suggest to run some further analysis (e.g., trying to segment into 4, 6 or 7 segments or rerunning the segmentation a few times to get rid of potential random errors) before taking decisions based on it. 
...
* Question 1)
* According to statistical arguments, there are 5 segments in our market. However, only 4 of the 5 segments are likely to buy a boat. Segment 4 does not seem very passionate about any of the things associated with boating. (See previous answers). So, we suggest to focus our marketing effort on the remaining 4 segments. 

* Question 2) 

* This question has aready been answered in Step 7 Question 2.
*
* Question 3) 

*The results change if we iterate; however, the results do not change dramatically, IE the business intuition stays the same. 

* Question 4) 
* We suggest to build different boats according to the needs of each segment. For example, the company could come up with value-brand that is cheaper but still has high quality. This brand could be marketed to segments that do not care about status (e.g., segment and 1 and 3)
*
*
